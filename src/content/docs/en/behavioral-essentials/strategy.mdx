...
- [State](/design-patterns/en/behavioral-essentials/state/) has the same structure but a different intent — its objects swap *themselves* as state changes, rather than being chosen by the client.
- [Template Method](/design-patterns/en/behavioral-essentials/template-method/) varies an algorithm through inheritance and overridden steps, where Strategy varies it through composition.
