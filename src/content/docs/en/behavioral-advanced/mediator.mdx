...
- [Observer](/design-patterns/en/behavioral-essentials/) also decouples senders from receivers, but it broadcasts to subscribers rather than centralizing arbitrary routing logic.
- [Facade](/design-patterns/en/structural/) likewise offers one point of contact, but a facade only simplifies one-way calls into a subsystem, whereas a mediator coordinates two-way traffic between peers.
